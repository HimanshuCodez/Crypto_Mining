        ...
        <div className='w-full flex flex-col gap-6 p-4 md:p-6 font-[Montserrat]'>
            <div className='flex flex-col justify-start items-start gap-2 font-[Inter]'>
                ...
                <nav className='flex items-center gap-1 capitalize font-light text-sm font-[Inter]'>
                    <a href="/Financial">Financial</a><span>/</span><a href="/Account Statment " className='text-[#02AC8F] truncate'>Account Statement</a>
                </nav>
            </div>
            <div className='bg-white w-full rounded-3xl px-4 md:px-5 py-10 flex flex-col gap-5'>
                <h2 className='text-xl md:text-2xl font-medium font-[Inter] px-3'>Account Statement</h2>
                <div className='bg-[#F7F7F7] py-3 px-2 md:px-8 pb-10 rounded-sm w-full overflow-x-auto'>
                    <div className='grid grid-cols-6 gap-6 text-sm md:text-lg font-medium min-w-[600px]'>
                        <div>#</div>
                        <div>Date</div>
                        <div>Mode</div>
                    ...
                    {transactions.map((transaction, index) => (
                        <div key={transaction._id} className='grid grid-cols-6 gap-4 py-3 text-gray-500 text-sm md:text-base min-w-[600px]'>
                            <div>{index + 1}</div>
