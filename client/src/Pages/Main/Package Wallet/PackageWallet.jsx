    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to send OTP. Please check details."
      );
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="w-full flex flex-col gap-6 p-4 md:p-10">
      <div className="flex flex-col justify-start items-start gap-2">
        <h2 className="text-2xl md:text-4xl font-medium capitalize font-[Inter]">
          Minning Investment Using Package Wallet
        </h2>
      </div>
      <div className="bg-[#FFFFFF] w-full rounded-3xl px-5 py-10 flex flex-col gap-5">
        <span className="font-[Inter] flex flex-col md:flex-row justify-start items-start gap-2">
          ...
          <h2 className="font-normal text-xl text-wrap font-[Inter] text-[#494949] ">
            You can Invest and Reinvest Using Package Wallet (Amount Should be
            In Multiples of $20).{" "}
          </h2>
        </span>
        <div className="grid grid-cols-1 md:grid-cols-2 justify-start items-center gap-6 md:gap-10 pt-8">
          <div className="font-[Inter] font-medium text-2xl flex flex-col gap-3 border rounded-lg p-3">
            ...
            <span className="text-[#2EB9A2]">
              ${user?.packageWallet?.toFixed(2) ?? "0.00"}
            </span>
          </div>
          <div className="font-[Inter] font-medium text-2xl flex flex-col gap-3 border rounded-lg p-3">
              ...
              <>
                ...
                >
                  ...
                  <input
                    ...
                    value={formData.otp}
                    onChange={handleChange}
                    className="outline-none w-full md:w-[20vw] border border-[#00000066] rounded-sm placeholder:text-[#000000B2] p-2"
                    id="otp"
                  />
                </label>
                <label
